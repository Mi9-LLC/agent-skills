...
**How to run.** Auto-triggers when a written plan exists and you ask for it to be reviewed, or run `/plan-eng-review`. `allowed-tools: Read, Grep, Glob, Bash, Write, Agent, WebSearch, WebFetch` (Bash is used read-only: git context, existence probes; Agent only for the optional outside-voice subagent; WebSearch/WebFetch only to verify external-library behavior against current docs).
