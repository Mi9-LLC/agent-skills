...
**What it does.** Reviews a Bitbucket pull request end to end and posts the summary back to the pull request, through the Composio MCP tools. Five things in order: fetch the pull request and its diffstat (following the pagination, and falling back to a local `git diff` when Bitbucket reports the diff as truncated); pin a throwaway checkout of the head commit so no line number in the review can move while it is being written, then cross-check that checkout's `--numstat` against Bitbucket's own diffstat and stop if the two disagree; run a parallel diff pass of four agents (two on `CLAUDE.md` and `.claude/rules/` compliance, two on bugs and security), where **every candidate finding then has to survive a separate validation subagent** and an unvalidated finding is dropped rather than reported; run a verification pass that treats every claim in the commit messages and the pull request description as a claim to settle against a primary source rather than as evidence, and that runs the target repository's own gates in the pinned checkout after installing its dependencies there; write the full report, then show the comment and ask before posting it.
