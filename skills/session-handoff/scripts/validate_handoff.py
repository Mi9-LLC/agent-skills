...
def _find_heading(content: str, section: str) -> re.Match | None:
    """Locate a markdown heading (levels 1-4) for the given section title.

    The handoff template nests sections like "Important Context" at level 3
    (###), so matching must accept any practical heading depth — the old
    `##?` pattern silently missed every ### heading.
    """
    pattern = rf'(?:^|\n)(#{{1,4}})\s+{re.escape(section)}'
...
def check_required_sections(content: str) -> tuple[bool, list[str]]:
    ...
    for section in REQUIRED_SECTIONS:
        match = _find_heading(content, section)
        if not match:
            ...
            # deeper subheadings (and their text) count as its content.
...
def check_recommended_sections(content: str) -> list[str]:
    ...
    for section in RECOMMENDED_SECTIONS:
        if not _find_heading(content, section):
...
def check_file_references(content: str, base_path: str) -> tuple[list[str], list[str]]:
    ...
    Two classes of false positive are skipped:
    - abbreviated paths containing ellipses (e.g. `docs/2026-...-proposal.md`),
      which are shorthand in prose, not real references;
    - paths on lines that describe the file as intentionally absent
      (deleted/renamed/superseded/etc.) — a handoff legitimately mentions
      files that no longer exist, and warning on them is noise.
    """
    ...
    # A line containing any of these is talking about a file that is
    # deliberately gone; its paths are not live references to verify.
    absent_markers = (
        'delet', 'renam', 'supersed', 'retired', 'no longer',
        'moved', 'does not exist', 'absorbed',
    )

    found_files = set()
    for line in content.splitlines():
        line_lower = line.lower()
        if any(marker in line_lower for marker in absent_markers):
            continue
        for pattern in patterns:
            for match in re.findall(pattern, line):
                # Remove line numbers
                filepath = match.split(':')[0]
                # Skip obvious non-files and abbreviated/ellipsis paths
                if not filepath or filepath.startswith('http') or '/' not in filepath:
                    continue
